...
class DoubleIGConvCmplx(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, no_g=4,
                 ...
                 last=True, bnorm='new', group=False):
        super().__init__()
...
class SFC(Model):
    ...
    def __init__(self, n_classes=10, n_channels=1, no_g=4, pooling='max', bnorm='old', final_gp='max', dropout=0.3, **kwargs):
        ...
        self.block1 = nn.Sequential(
            IGConvCmplx(n_channels, 24, 9, no_g, padding=2, weight_init='he'),
            ReLUCmplx(inplace=True),
            BatchNormCmplx((24 * no_g), bnorm_type=bnorm))
        self.block2 = nn.Sequential(
            IGConvGroupCmplx(24, 32, 7, no_g, padding=3, weight_init='he'),
            ReLUCmplx(inplace=True),
            BatchNormCmplx((32 * no_g), bnorm_type=bnorm))
        self.block3 = nn.Sequential(
            IGConvGroupCmplx(32, 36, 7, no_g, padding=3, weight_init='he'),
            ReLUCmplx(inplace=True),
            BatchNormCmplx((36 * no_g), bnorm_type=bnorm))
        self.block4 = nn.Sequential(
            IGConvGroupCmplx(36, 36, 7, no_g, padding=3, weight_init='he'),
            ReLUCmplx(inplace=True),
            BatchNormCmplx((36 * no_g), bnorm_type=bnorm))
        self.block5 = nn.Sequential(
            IGConvGroupCmplx(36, 64, 7, no_g, padding=2, weight_init='he'),
            ReLUCmplx(inplace=True),
            BatchNormCmplx((64 * no_g), bnorm_type=bnorm))
        self.block6 = nn.Sequential(
            IGConvGroupCmplx(64, 96, 5, no_g, padding=1, weight_init='he'),
            ReLUCmplx(inplace=True),
