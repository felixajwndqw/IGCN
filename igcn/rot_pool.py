...
class RotMaxPool2d(_MaxPoolNd):
    ...
    def rotate(self, input, thetas):
        ...
        for i, th in enumerate(thetas.unsqueeze(1)):
            M = tgm.get_rotation_matrix2d(centre, th.repeat(input.size(0)), scale) #.repeat(input.size(0), 2, 3)
            print(centre.size(), th.size(), scale.size(), M.size())
            print(input.size(), input[:, i*rot_batch:(i+1)*rot_batch].size())
