...
class TripleConvCmplx(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, relu_type='mod', **kwargs):
        super().__init__()
...
class DownCmplx(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, pooling='avg', **kwargs):
        super().__init__()
        if pooling == 'avg':
            Pool = AvgPoolCmplx(2)
        elif pooling == 'mag':
        ...
        self.pool_conv = nn.Sequential(
            Pool,
            TripleConvCmplx(in_channels, out_channels, kernel_size, **kwargs)
        )
...
class UpCmplx(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, upsample_mode='nearest', **kwargs):
        super().__init__()
        ...
        self.conv = TripleConvCmplx(in_channels, out_channels, kernel_size, **kwargs)
...
class UNet(Model):
    def __init__(self, n_classes, n_channels=1, base_channels=16,
                 kernel_size=3, nfc=1, dropout=0., pooling='max', mode='nearest', pad_to_remove=64, **kwargs):
        super().__init__(**kwargs)
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.mode = mode
...
class Down(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, pooling='avg'):
        ...
        self.pool_conv = nn.Sequential(
            Pool(2),
            TripleConv(in_channels, out_channels, kernel_size)
        )
...
class Up(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, mode='nearest'):
        ...
        self.conv = TripleConv(in_channels, out_channels, kernel_size)
